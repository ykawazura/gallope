...
module params
  ...
  logical :: shear
  real(8) :: q
  !$acc declare create(nu, eta)
  !$acc declare create(nu_h, eta_h)
  !$acc declare create(nu_h_exp, eta_h_exp)
...
!-----------------------------------------------!
  subroutine read_parameters(filename)
    ...
    !$acc update device(nu, eta)
    !$acc update device(nu_h, eta_h)
    !$acc update device(nu_h_exp, eta_h_exp)
    !$acc update device(shear, q)
