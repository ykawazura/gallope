...
!-----------------------------------------------!
  subroutine solve
    ...
    if(time_step_scheme == 'eSSPIFRK3') then
      ...
      if(nonlinear) call get_nonlinear_terms_g(g)
          ...
          do j = 1, nky_local
            do k = 1, nkz
              ...
              call eSSPIFRK1(g_tmp(k,j,i,mm), g(k,j,i,mm), &
                 exp_terms_g(k,j,i,mm), imp_g1, imp_g0)
              g_tmp(k,j,i,mm) = g_tmp(k,j,i,mm)*filter(k,j,i)
            enddo
          enddo
        enddo
      enddo
          ...
          do j = 1, nky_local
            do k = 1, nkz
              ...
              call eSSPIFRK2(g_tmp(k,j,i,mm), g_tmp(k,j,i,mm), g(k,j,i,mm), &
                 exp_terms_g(k,j,i,mm), imp_g2, imp_g0)
              g_tmp(k,j,i,mm) = g_tmp(k,j,i,mm)*filter(k,j,i)
            enddo
          enddo
        enddo
        ...
        do i = 1, nkx
          do j = 1, nky_local
            do k = 1, nkz
              ...
              call eSSPIFRK3(g_new(k,j,i,mm), g_tmp(k,j,i,mm), g(k,j,i,mm), &
                 exp_terms_g(k,j,i,mm), exp_terms0_g(k,j,i,mm), &
                 imp_g3, imp_g2, imp_g0)
              g_new(k,j,i,mm) = g_new(k,j,i,mm)*filter(k,j,i)
            enddo
          enddo
        enddo
      enddo
      ...
      ! ---- g: commit the advanced moments into the state array ----
      !$acc data present(g, g_new)
      !$acc parallel loop collapse(4)
      do mm = 1, nm_local
        do i = 1, nkx
          do j = 1, nky_local
            do k = 1, nkz
              g(k,j,i,mm) = g_new(k,j,i,mm)
            enddo
          enddo
        enddo
      enddo
...
!-----------------------------------------------!
  subroutine get_nonlinear_terms_g(g_in)
    ...
    do mm = 1, nm_local
      ...
      ! 4. Forward FFT + normalization
      call ftran_r2c(nonlin_r_g(:,:,:), nonlin_g(:,:,:,mm))
      !$acc data present(nonlin_g)
      !$acc parallel loop collapse(3)
      do i = 1, nkx
        do j = 1, nky_local
          do k = 1, nkz
            nonlin_g(k,j,i,mm) = nonlin_g(k,j,i,mm)/ntot
          enddo
        enddo
      enddo
      !$acc end data
...
!-----------------------------------------------!
!> @author  YK
!! @brief   Time integral of the diagonal linear dissipation of g_m:
!!          perpendicular hyperviscosity mu*(k_perp^2/k_perp^2_max)^nexp_perp
!!          and Hermite hypercollision nu*(m/nm)^nexp_m (m=0 is undamped).
!!          Same sign convention as get_imp_terms_tintg (integrating factor).
!-----------------------------------------------!
  subroutine get_imp_terms_tintg_g(imp_terms_tintg, t, kprp2, m_glob)
    !$acc routine seq
    use grid, only: kprp2_max, nm
    use params, only: mu_hyper_perp, nu_hyper_m, nexp_perp, nexp_m
    implicit none
    real(8), intent(out) :: imp_terms_tintg
    real(8), intent(in)  :: t, kprp2
    integer, intent(in)  :: m_glob

    imp_terms_tintg = -( mu_hyper_perp*(kprp2/kprp2_max)**nexp_perp &
                       + nu_hyper_m*(dble(m_glob)/dble(nm))**nexp_m )*t
  end subroutine get_imp_terms_tintg_g
...
!-----------------------------------------------!
  subroutine allocate_advance
    ...
    if(time_step_scheme == 'eSSPIFRK3') then
      ...
      ! Hermite-moment (g) work arrays. g_tmp/g carry m-ghosts (0:nm_local+1);
      ! the other work arrays are interior-only (1:nm_local).
      allocate(g_new       (nkz       , nky_local, nkx      , nm_local     ), source=(0.d0, 0.d0))
      ...
      !$acc enter data copyin(g_tmp)
      !$acc enter data copyin(exp_terms_g)
      !$acc enter data copyin(exp_terms0_g)
      !$acc enter data copyin(wg)
      !$acc enter data copyin(nonlin_g)
      !$acc enter data copyin(wg_r)
      !$acc enter data copyin(nonlin_r_g)
    endif
...
!-----------------------------------------------!
  subroutine deallocate_advance
    ...
    if(time_step_scheme == 'eSSPIFRK3') then
      ...
      !$acc exit data delete(g_new)
      !$acc exit data delete(g_tmp)
      !$acc exit data delete(exp_terms_g)
      !$acc exit data delete(exp_terms0_g)
      !$acc exit data delete(wg)
      !$acc exit data delete(nonlin_g)
      !$acc exit data delete(wg_r)
      !$acc exit data delete(nonlin_r_g)
      deallocate(g_new)
      deallocate(g_tmp)
      deallocate(exp_terms_g)
      deallocate(exp_terms0_g)
      deallocate(wg)
      deallocate(nonlin_g)
      deallocate(wg_r)
      deallocate(nonlin_r_g)
    endif
