...
at the root directory will copy 
  gallope.out.std -> gallope.out.std1
  gallope.in      -> gallope.in1
  gallope.out.nc  -> gallope.out.nc1
  out2d           -> out2d1
  restart         -> restart1
and so on.
...
if restart_num != 'all':
  ...
  flg = False
  for filename_expand in sorted(glob.glob('gallope.modes.out?*'), key=natural_keys):
    flg = True
    command += filename_expand + ' '

  command += '> gallope.modes.out_all'
