...
kp_log_kpar_end = int(kpbin_log_kpar.size*2./3.)
...
def fold_in_ky(u):
  unew = [(u[:, i, :] + u[:, -i, :])/2 for i in np.arange(0, int(nky/2)+1)]
...
nky_mid = np.where(np.sign(ky) == -1)[0][0]
kx_ = np.concatenate([kx[nkx_mid:], kx[0:nkx_mid]])
ky_ = np.concatenate([ky[nky_mid:], ky[0:nky_mid]])

u2_kxy_sum_kz = np.transpose(np.fromfile(input_dir+'out2d'+restart_num+'/u2_kxy_sum_kz.dat').reshape(nt_2d, nky, nkx)); u2_kxy_sum_kz = np.transpose(u2_kxy_sum_kz, axes=(2, 1, 0));  u2_kxy_sum_kz = np.delete(u2_kxy_sum_kz, ignored_points_2d, axis = 0); 
u2_kxy_sum_kz = np.concatenate([u2_kxy_sum_kz[:, nky_mid:, :], u2_kxy_sum_kz[:, 0:nky_mid, :]], axis=1)
u2_kxy_sum_kz = np.concatenate([u2_kxy_sum_kz[:, :, nkx_mid:], u2_kxy_sum_kz[:, :, 0:nkx_mid]], axis=2)
u2_kyz_sum_kx = np.transpose(np.fromfile(input_dir+'out2d'+restart_num+'/u2_kyz_sum_kx.dat').reshape(nt_2d, nkz, nky)); u2_kyz_sum_kx = np.transpose(u2_kyz_sum_kx, axes=(2, 1, 0));  u2_kyz_sum_kx = np.delete(u2_kyz_sum_kx, ignored_points_2d, axis = 0); 
u2_kyz_sum_kx = np.concatenate([u2_kyz_sum_kx[:, :, nky_mid:], u2_kyz_sum_kx[:, :, 0:nky_mid]], axis=2)
u2_kxz_sum_ky = np.transpose(np.fromfile(input_dir+'out2d'+restart_num+'/u2_kxz_sum_ky.dat').reshape(nt_2d, nkz, nkx)); u2_kxz_sum_ky = np.transpose(u2_kxz_sum_ky, axes=(2, 1, 0));  u2_kxz_sum_ky = np.delete(u2_kxz_sum_ky, ignored_points_2d, axis = 0); 
u2_kxz_sum_ky = np.concatenate([u2_kxz_sum_ky[:, :, nkx_mid:], u2_kxz_sum_ky[:, :, 0:nkx_mid]], axis=2)

b2_kxy_sum_kz = np.transpose(np.fromfile(input_dir+'out2d'+restart_num+'/b2_kxy_sum_kz.dat').reshape(nt_2d, nky, nkx)); b2_kxy_sum_kz = np.transpose(b2_kxy_sum_kz, axes=(2, 1, 0));  b2_kxy_sum_kz = np.delete(b2_kxy_sum_kz, ignored_points_2d, axis = 0); 
b2_kxy_sum_kz = np.concatenate([b2_kxy_sum_kz[:, nky_mid:, :], b2_kxy_sum_kz[:, 0:nky_mid, :]], axis=1)
b2_kxy_sum_kz = np.concatenate([b2_kxy_sum_kz[:, :, nkx_mid:], b2_kxy_sum_kz[:, :, 0:nkx_mid]], axis=2)
b2_kyz_sum_kx = np.transpose(np.fromfile(input_dir+'out2d'+restart_num+'/b2_kyz_sum_kx.dat').reshape(nt_2d, nkz, nky)); b2_kyz_sum_kx = np.transpose(b2_kyz_sum_kx, axes=(2, 1, 0));  b2_kyz_sum_kx = np.delete(b2_kyz_sum_kx, ignored_points_2d, axis = 0); 
b2_kyz_sum_kx = np.concatenate([b2_kyz_sum_kx[:, :, nky_mid:], b2_kyz_sum_kx[:, :, 0:nky_mid]], axis=2)
b2_kxz_sum_ky = np.transpose(np.fromfile(input_dir+'out2d'+restart_num+'/b2_kxz_sum_ky.dat').reshape(nt_2d, nkz, nkx)); b2_kxz_sum_ky = np.transpose(b2_kxz_sum_ky, axes=(2, 1, 0));  b2_kxz_sum_ky = np.delete(b2_kxz_sum_ky, ignored_points_2d, axis = 0); 
b2_kxz_sum_ky = np.concatenate([b2_kxz_sum_ky[:, :, nkx_mid:], b2_kxz_sum_ky[:, :, 0:nkx_mid]], axis=2)

plot_2d(np.log10(u2_kxy_sum_kz[final_idx,1:-1,1:-1]), kx_[1:-1], ky_[1:-1], xlab=r'$k_x$', ylab=r'$k_y$', title=r'$E_u (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, aspect='auto', save=outdir+'u2_kxy.pdf')
plot_2d(np.log10(u2_kyz_sum_kx[final_idx,1:-1,1:-1]), ky_[1:-1], kz [1:-1], xlab=r'$k_y$', ylab=r'$k_z$', title=r'$E_u (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, aspect='auto', save=outdir+'u2_kyz.pdf')
plot_2d(np.log10(u2_kxz_sum_ky[final_idx,1:-1,1:-1]), kx_[1:-1], kz [1:-1], xlab=r'$k_x$', ylab=r'$k_z$', title=r'$E_u (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, aspect='auto', save=outdir+'u2_kxz.pdf')
plot_2d(np.log10(b2_kxy_sum_kz[final_idx,1:-1,1:-1]), kx_[1:-1], ky_[1:-1], xlab=r'$k_x$', ylab=r'$k_y$', title=r'$E_B (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, aspect='auto', save=outdir+'b2_kxy.pdf')
plot_2d(np.log10(b2_kyz_sum_kx[final_idx,1:-1,1:-1]), ky_[1:-1], kz [1:-1], xlab=r'$k_y$', ylab=r'$k_z$', title=r'$E_B (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, aspect='auto', save=outdir+'b2_kyz.pdf')
plot_2d(np.log10(b2_kxz_sum_ky[final_idx,1:-1,1:-1]), kx_[1:-1], kz [1:-1], xlab=r'$k_x$', ylab=r'$k_z$', title=r'$E_B (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, aspect='auto', save=outdir+'b2_kxz.pdf')


# bin over (kx, ky) leaving kz or (kx, kz) leaving ky
u2_kxy_vs_kz = np.fromfile(input_dir+'out2d'+restart_num+'/u2_kxy_vs_kz.dat').reshape(nt_2d, nkz, nkpolar)
u2_kxz_vs_ky = np.fromfile(input_dir+'out2d'+restart_num+'/u2_kxz_vs_ky.dat').reshape(nt_2d, nky, nkpolar); u2_kxz_vs_ky = fold_in_ky(u2_kxz_vs_ky)
b2_kxy_vs_kz = np.fromfile(input_dir+'out2d'+restart_num+'/b2_kxy_vs_kz.dat').reshape(nt_2d, nkz, nkpolar)
b2_kxz_vs_ky = np.fromfile(input_dir+'out2d'+restart_num+'/b2_kxz_vs_ky.dat').reshape(nt_2d, nky, nkpolar); b2_kxz_vs_ky = fold_in_ky(b2_kxz_vs_ky)

plot_log2d(u2_kxy_vs_kz[final_idx,1:nkz - 1,1:kp_end], kpbin[1:kp_end], kz[1:nkz - 1] , xlab=r'$k_\+$', ylab=r'$k_z$', title=r'$E_u (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, save=outdir+'u2_kxy_vs_kz.pdf')
plot_log2d(b2_kxy_vs_kz[final_idx,1:nkz - 1,1:kp_end], kpbin[1:kp_end], kz[1:nkz - 1] , xlab=r'$k_\+$', ylab=r'$k_z$', title=r'$E_u (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, save=outdir+'b2_kxy_vs_kz.pdf')

plot_log2d(u2_kxz_vs_ky[final_idx,1:int(nky/2),1:kp_end], kpbin[1:kp_end], ky[1:int(nky/2)] , xlab=r'$k_\+$', ylab=r'$k_y$', title=r'$E_B (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, save=outdir+'u2_kxz_vs_ky.pdf')
plot_log2d(b2_kxz_vs_ky[final_idx,1:int(nky/2),1:kp_end], kpbin[1:kp_end], ky[1:int(nky/2)] , xlab=r'$k_\+$', ylab=r'$k_y$', title=r'$E_B (t = %.2E) $' % tt_2d[final_2d_idx], contour=True, save=outdir+'b2_kxz_vs_ky.pdf')
