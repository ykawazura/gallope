...
!-----------------------------------------------!
  subroutine loop_diagnostics
    ...
    ! source= must match the type of src, which is real(8): the complex literal
    ! (0.d0, 0.d0) that stood here was accepted by the nvfortran 25.9 on Miyabi
    ! but is rejected by 26.1.  The array is zero either way, so this changes
    ! nothing that was measured -- it only lets the same source build here.
